...
exports.postProducts = async (req,res)=>{
  try {
    ...
    const { sub_category_id, model, product_name, color, price } = JSON.parse(await req.body);
...
exports.deleteProducts = async (req, res) => {
   try {
     ...
     const unDelCategories = products.filter(
...
if (req.query.sub_category_id) {
  if (req.query && req.query.sub_category_id != "undifined") { 
    const subCategoryFilter = await subCategories.filter( (items) => items.sub_category_id == req.query.sub_category_id
    );
    subCategoryFilter.map((subId)=>{
      subId.categor = category.find((categor)=>categor.category_id == category.category_id)
      subId.product = products.find((product) => product.sub_category_id == products.sub_category_id );
    })
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if (req.query.model) {
   if (req.query && req.query.model != "undifined") {
        const productFilter = products.filter( (items) => items.model == req.query.model );
        productFilter.map((productId) => {
            productId.categor = category.find((categor) => categor.category_id == productId.category_id  );
            productId.subCategor = subCategories.find((subCategor) => subCategor.sub_category_id == productId.sub_category_id  );
          });
           res.setHeader("Access-control-allow-origin", "*");
           res.writeHead(200, { "content-type": "application/json" });
    ...
    if (req.query.category_id) {
        if (req.query && req.query.category_id != "undifined") {
          const CategoryFilter =  category.filter(
            (items) => items.category_id == req.query.category_id
          );
          CategoryFilter.map((categoryId) => {
            categoryId.categor = subCategories.find((categor) => categor.sub_category_id == categoryId.sub_category_id );
            categoryId.product = products.find((product) => product.sub_category_id == categoryId.sub_category_id );
          });
          res.setHeader("Access-control-allow-origin", "*");
          res.writeHead(200, { "content-type": "application/json" });
